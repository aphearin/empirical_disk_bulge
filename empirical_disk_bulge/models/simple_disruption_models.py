...
def time_dependent_disruption(sfr_history, sm_history, cosmic_age_array, zobs,
        frac_migration, probt1, probt2, t1=1.5, t2=13.8):
    """
    Examples
    --------
    >>> ngals, ntimes = 100, 178
    ...
    >>> zobs = 0.1
    >>> frac_migration = 0.25
    >>> probt1, probt2 = 0.05, 0.01
    >>> sm_disk, sm_bulge = time_dependent_disruption(sfr_history, sm_history, cosmic_age_array, zobs, frac_migration, probt1, probt2)
    """
    num_gals = np.shape(sfr_history)[0]
    prob_array = np.array([np.interp(t, [t1, t2], [probt1, probt2]) for t in cosmic_age_array])
